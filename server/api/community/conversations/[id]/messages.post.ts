...
export default defineEventHandler(async (event) => {
  ...
  if (!canRead(conv, me.id))
    throw createError({ statusCode: 403, statusMessage: 'Bạn không có quyền gửi tin nhắn ở đây' })

  const payload = (await readBody(event)) || {}
  const body = String(payload.body || '').trim()
  const type = payload.type === 'system' ? 'system' : 'text'

  if (!body) throw createError({ statusCode: 400, statusMessage: 'Nội dung tin nhắn không được để trống' })
  if (body.length > 2000)
    throw createError({ statusCode: 400, statusMessage: 'Tin nhắn tối đa 2000 ký tự' })
