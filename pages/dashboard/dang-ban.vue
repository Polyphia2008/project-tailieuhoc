...
const page = ref(1)
const { data, pending } = await useFetch<any>('/api/user/documents', { query: computed(() => ({ page: page.value, limit: 12, status: undefined })) })
useHead({ title: 'Tài liệu đang bán - MapDocs' })
</script>
<template>
  <div>
    <nav class="flex items-center gap-1.5 text-[12px] text-mdk-mute"><NuxtLink to="/dashboard" class="hover:text-mdk-sub">Dashboard</NuxtLink> / <span class="text-mdk-sub">Tài liệu đang bán</span></nav>
    <h1 class="mt-3 text-[22px] font-bold text-mdk-text font-ui tracking-tight">Tài liệu đang bán</h1>
