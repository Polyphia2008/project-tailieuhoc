...
const { data, pending, refresh } = await useFetch<any>('/api/admin/users', { query: computed(() => ({ page: page.value, limit: 15, q: q.value || undefined, role: filter || undefined })) })
...
async function act(action: string, extra: any = {}) {
  ...
  try { const r = await $fetch<any>('/api/admin/users', { method: 'POST', body: { action, ids: sel.value, ...extra } }); sel.value = []; await refresh(); toast.success(`Đã xử lý ${r.affected ?? 1} bản ghi`) }
  catch (e: any) { toast.error(e?.data?.statusMessage || 'Lỗi') }
}
watch(filter, () => { page.value = 1 })
useHead({ title: 'Quản lý người dùng - MapDocs Admin' })
</script>
<template>
  <div>
    <nav class="flex items-center gap-1.5 text-[12px] text-mdk-mute"><NuxtLink to="/admin" class="hover:text-mdk-sub">Admin</NuxtLink> / <span class="text-mdk-sub">Quản lý người dùng</span></nav>
    <h1 class="mt-3 text-[22px] font-bold text-mdk-text font-ui tracking-tight">Quản lý người dùng</h1>
