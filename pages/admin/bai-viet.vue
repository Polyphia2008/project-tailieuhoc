...
const { data, pending, refresh } = await useFetch<any>('/api/admin/blogs', { query: computed(() => ({ page: page.value, limit: 15, q: q.value || undefined, published: filter || undefined })) })
...
async function act(action: string, extra: any = {}) {
  ...
  try { const r = await $fetch<any>('/api/admin/blogs', { method: 'POST', body: { action, ids: sel.value, ...extra } }); sel.value = []; await refresh(); toast.success(`Đã xử lý ${r.affected ?? 1} bản ghi`) }
...
<template>
  <div>
    <nav class="flex items-center gap-1.5 text-[12px] text-mdk-mute"><NuxtLink to="/admin" class="hover:text-mdk-sub">Admin</NuxtLink> / <span class="text-mdk-sub">Bài viết</span></nav>
    <h1 class="mt-3 text-[22px] font-bold text-mdk-text font-ui tracking-tight">Bài viết</h1>
